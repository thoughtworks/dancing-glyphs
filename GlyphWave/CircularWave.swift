...
class CircularWave: Wave
{
    ...
    {
        var list: [Sprite] = []
        let step = Float(2.0 * .pi) / Float(numSprites)
    ...
    static func move(sprite s: Sprite, to nowIn: Double)
    {
        ...
        y += sin(now * 1.5 + Double(s.anchor.x) * .pi * 1.0) * 0.04   //  large wave across sprites
        ...
        s.rotation = Float(sin(now * (s.r0-0.5)) * 2 * .pi)         // rotation based on r0
    }
